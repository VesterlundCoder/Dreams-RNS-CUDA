#!/usr/bin/env python3
"""
Odd-Zeta CMF Explorer: exact irrationality measurement for ζ(2n+1) CMFs.

Uses EXACT big-integer arithmetic — no float, no RNS approximation.
At each step k, the matrix M(k) is evaluated as exact rationals, scaled
to integers by denominator clearing, and the state vector is multiplied
exactly.  After N steps the accumulator p and denominator q are exact
Python ints.  δ = -(1 + log|p/q - ζ| / log|q|) is computed with mpmath
at arbitrary precision.

This is the tool for probing δ > 0 territory — irrationality-proof grade.

Usage:
    python odd_zeta_sweep.py --specs odd_zeta_specs.jsonl --depth 5000 --shifts 3 --max-cmfs 1
    python odd_zeta_sweep.py --specs odd_zeta_specs.jsonl --depth 5000 --shifts 1
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Odd-Zeta CMF Explorer — exact big-integer irrationality measurement"
    )
    ...
    print(f"\nOdd-Zeta CMF Explorer — EXACT big-integer arithmetic")
